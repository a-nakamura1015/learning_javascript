...
const alphabetical = words.reduce((a,x) => {
    if(!a[x[0]]) // 先頭文字のプロパティがあるかをチェック
        a[x[0]] = []; // なければ空で作成
        a[x[0]].push(x); // 現在の要素を記憶
        // console.log(a[x[0]]); // 途中経過確認用
        return a}, {});
...
// 別の例
const 単語リスト = ["ビーチボール","ルービックキューブ","ブタベスト","トロッコ","コンゴ","ゴースト","トーラス","スマート","トンガ",
                    "ガラパゴス","ストリート","トーマス","ストレッチ","チベット","トキ","キツツキ","キリン"];
